...
void
kvminithart_proc(pagetable_t pagetable)
{
...
void
u2kvmcopy(pagetable_t pagetable, pagetable_t kel_pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for(uint64 i = oldsz; i < newsz; i += PGSIZE)
  {
    ...
    if((pte_to = walk(kel_pagetable, i, 1)) == 0) {
